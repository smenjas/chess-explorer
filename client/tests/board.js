...
tests['Board.decode() works.'] = () => {
    const board = new Board();
    const actualHash = board.encode();
    const expectedHash = 'rnbqkbnrpppppppp                                PPPPPPPPRNBQKBNR fW';
    ...
    if (actualHash !== expectedHash) {
        return ['Starting position hash differs unexpectedly'];
    }
    const tests = [
        [[actualHash], expectedPosition],
    ];
    return Test.run(Board.decode, tests);
};
